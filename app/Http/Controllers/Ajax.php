    ...
    */
    public function getGames()
    {
        ...
        foreach ($games as $game) {
            ...
            $json_games[$game->id]['home']['team'] = DB::table('team')->select('name','abbr')->where('id',$game->home_team_id)->get();
            $json_games[$game->id]['away']['score'] = DB::table('score')->select('away_q1')->where('game_id',$game->id)->value('away_q1');
            $json_games[$game->id]['away']['team'] = DB::table('team')->select('name','abbr')->where('id',$game->away_team_id)->get();
        }
